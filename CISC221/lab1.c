...
void chkendian(){
	...
	/* why wouldnt the following work?
		answer: endianness does not affect the outcome of variable values, only the way values are stored.
...
int le2bep(int x){
       int d = x;			//d = 0x12345678
       char *dptr = &d;			//dptr = 0x10000
       int bed;				
       char *bedptr = &bed;		//bedptr = 0x11000
       *(bedptr + 3) = *dptr;		//*(bedptr + 3) = 0x11012
       *(bedptr + 2) = *(dptr + 1);	//*(bedptr + 2) = 0x11008
       *(bedptr + 1) = *(dptr + 2);	//*(bedptr + 1) = 0x11004
       *bedptr = *(dptr + 3);		//*bedptr = 0x10012
       printf('%d', *(bedptr + 3);
       return bed;			//bed = 0x0
